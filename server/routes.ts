...
import { storage, generateSalt } from "./storage";
...
import { setupAuth, isAuthenticated, getUserIdFromRequest } from "./auth";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
      ...
      const newUser = await storage.createUser({
        ...
        email,
        password,
        salt: '' // Will be generated in createUser method
      });
      
      // Initialize user data
      await storage.createSurvey(newUser.id, { completed: false, lastStep: 0 });
  ...
  app.post("/api/user/survey/complete", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = getUserIdFromRequest(req);
  ...
  // Initialize WebSocket server for chat with session support
